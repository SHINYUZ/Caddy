...
# Author: Shinyuz | Fix: Spacing, Symlink
# =========================================================
...
# --- 工具函数：设置快捷键 (修复死链问题) ---
set_shortcut() {
    # 获取脚本的绝对路径，解决 dangling symlink 问题
    local current_path
    current_path=$(readlink -f "$0")

    # 如果快捷键不存在，或者指向的路径不对，则重新创建
    if [[ ! -f /usr/bin/ca ]] || [[ "$(readlink -f /usr/bin/ca)" != "$current_path" ]]; then
        ln -sf "$current_path" /usr/bin/ca
        chmod +x "$current_path"
        echo ""
        echo -e "${GREEN}快捷键设置成功！输入 'ca' 即可随时呼出脚本${PLAIN}"
        # 此处不加 echo ""，由 show_menu 统一控制间距
    fi
}
